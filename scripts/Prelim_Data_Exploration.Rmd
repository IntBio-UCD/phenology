...
UCD_pheno_transplant$last.FL.date <- mdy(UCD_pheno_transplant$last.FL.date) #no last flower dates
UCD_pheno_transplant$last.FR.date <- mdy(UCD_pheno_transplant$last.FR.date) #no last fruit dates
```
...
colnames(clim_dist_UCD)[colnames(clim_dist_UCD) == "parent.pop"] <- "pop"
...
m4 <- glmer(initiate_repro ~ Gowers_Dist + (1 | pop),
            data=UCD_24_model,
            family=binomial)
summary(m4)
...
```{r}
m10 <- glmer(initiate_repro ~ (1 | pop),
             data=UCD_24_model,
             family=binomial)
summary(m10)
```
...
print(WL224_lastfl_no_lastflw) #what do I do about this? set last fl the same as last fr?
```
...
colnames(clim_dist_WL2)[colnames(clim_dist_WL2) == "parent.pop"] <- "pop"
