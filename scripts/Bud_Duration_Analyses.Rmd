...
```
```{r}
WL2_24_bud_dur_summary_clim_dist <- init.repro%>%
  ...
  filter(N_bud > 1) %>%
  left_join(WL2_24_bud_dur %>% 
      select(pop, GrwSsn_GD, Wtr_Year_GD) %>% 
      distinct(),
    by = "pop")

ggplot(WL2_24_bud_dur_summary_clim_dist, aes(x=pop, y=mean_bud_dur, color=elev_m)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_bud_dur - sem_bud_dur, ymax = mean_bud_dur + sem_bud_dur), width = 0.02) +
  scale_color_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Bud Duration")

ggplot(WL2_24_bud_dur_summary_clim_dist, aes(x=GrwSsn_GD, y=mean_bud_dur, color=elev_m)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_bud_dur - sem_bud_dur, ymax = mean_bud_dur + sem_bud_dur), width = 0.02) +
  scale_color_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Bud Duration")

ggplot(WL2_24_bud_dur_summary_clim_dist, aes(x=Wtr_Year_GD, y=mean_bud_dur, color=elev_m)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_bud_dur - sem_bud_dur, ymax = mean_bud_dur + sem_bud_dur), width = 0.02) +
  scale_color_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Bud Duration")
