...
pop_order <- WL2_24_dt_bud %>%
  distinct(pop, elev_m) %>%
  arrange(elev_m) %>%
  pull(pop)

WL2_24_dt_bud <- WL2_24_dt_bud %>%
...
GrwSsn_order <- WL2_24_dt_bud %>%
  distinct(pop, GrwSsn_GD) %>%
  arrange(GrwSsn_GD) %>%
  pull(pop)

WL2_24_dt_bud <- WL2_24_dt_bud %>%
  mutate(pop = factor(pop, levels = GrwSsn_order))
...
Wtr_Year_order <- WL2_24_dt_bud %>%
  distinct(pop, Wtr_Year_GD) %>%
  arrange(Wtr_Year_GD) %>%
  pull(pop)

WL2_24_dt_bud <- WL2_24_dt_bud %>%
  mutate(pop = factor(pop, levels = Wtr_Year_order))
...
WL2_24_dt_bud_summary_clim_dist <- init.repro%>%
  filter(site == "WL2" & year == 24) %>%
  group_by(pop, site, elev_m) %>%
  summarize(N_bud = sum(!is.na(bud.date_dt)),
            mean_dt_bud = mean(bud.date_dt, na.rm=(TRUE)),
            sem_dt_bud = sem(bud.date_dt, na.rm=(TRUE))
            ) %>%
  left_join(WL2_24_dt_bud %>% 
      select(pop, GrwSsn_GD, Wtr_Year_GD) %>% 
      distinct(),
    by = "pop")

ggplot(WL2_24_dt_bud_summary_clim_dist, aes(x=GrwSsn_GD, y=mean_dt_bud, color=elev_m)) +
  geom_point(size = 3) +
...
ggplot(WL2_24_dt_bud_summary_clim_dist, aes(x=Wtr_Year_GD, y=mean_dt_bud, color=elev_m)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_dt_bud - sem_dt_bud, ymax = mean_dt_bud + sem_dt_bud), width = 0.02) +
  scale_color_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Days to bud")
...
GrwSsn_order <- UCD_dt_bud %>%
  distinct(pop, GrwSsn_GD) %>%
  arrange(GrwSsn_GD) %>%
  pull(pop)

UCD_dt_bud <- UCD_dt_bud %>%
  mutate(pop = factor(pop, levels = GrwSsn_order))
...
UCD_dt_bud_summary_clim_dist <- dt_bud_summary %>%
  filter(site == "UCD") %>%
   left_join(UCD_dt_bud %>% 
      select(pop, GrwSsn_GD, Wtr_Year_GD) %>% 
      distinct(),
    by = "pop")

ggplot(UCD_dt_bud_summary_clim_dist, aes(x=GrwSsn_GD, y=mean_dt_bud, color=elev_m)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_dt_bud - sem_dt_bud, ymax = mean_dt_bud + sem_dt_bud), width = 0.02) +
  scale_color_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Days to bud")

ggplot(UCD_dt_bud_summary_clim_dist, aes(x=Wtr_Year_GD, y=mean_dt_bud, color=elev_m)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_dt_bud - sem_dt_bud, ymax = mean_dt_bud + sem_dt_bud), width = 0.02) +
  scale_color_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Days to bud")
