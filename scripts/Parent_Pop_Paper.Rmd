...
#### Bud duration and timing at UCD and number of fruits/FrFlN (only TM2, DPR, and BH had more than 1 individual with fruit/FrFlN)
...
UCD_bud_fruit <- UCD_bud_fruit %>%
  ...
  filter(!(is.na(bud.duration)))%>%
  filter(!(pop %in% c("SC", "CP2", "SQ3", "WL2")))
...
ggplot(UCD_bud_fruit, aes(x=bud.date_dt, y=fruits, color=pop))+
  geom_point() #visually, more fruits were produced when budding started earlier although tthis could be driven by TM2 as well

UCD_dt_bud_fruit <- lmer(fruits ~ bud.date_dt + (1|pop),
                         data=UCD_bud_fruit)
summary(UCD_dt_bud_fruit)
Anova(UCD_dt_bud_fruit)
...
ggplot(WL2_bud_dur_fit, aes(x=bud.duration, y=fruits, color=elev_m))+
  geom_point() +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  labs(x="Bud Duration", y="Fruit Number")
...
ggplot(WL2_dt_bud_fruit, aes(x=bud.date_dt, y=fruits, color=elev_m))+
  geom_point() +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  labs(x="Days to bud", y="Fruit Number")
...
summary(WL2_bud_dur_FrFlN) # negative relationship between bud duration and fruit number
...
```{r}
bud_seq <- seq(min(WL2_bud_dur_fit$bud.duration, na.rm = TRUE),
               max(WL2_bud_dur_fit$bud.duration, na.rm = TRUE))

newdata <- data.frame(bud.duration = bud_seq)

newdata$predicted <- predict(WL2_bud_dur_fruit, newdata = newdata, re.form = NA)

ggplot(WL2_bud_dur_fit, aes(x = bud.duration, y = fruits, color = elev_m)) +
  geom_point(alpha = 0.7) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata, aes(x = bud.duration, y = predicted), color = "blue", size = 1.2) +
  labs(x = "Bud Duration", y = "Fruit Number")

dt_bud_seq <- seq(min(WL2_dt_bud_fruit$bud.date_dt, na.rm = TRUE),
                  max(WL2_dt_bud_fruit$bud.date_dt, na.rm = TRUE))

newdata_dt <- data.frame(bud.date_dt = dt_bud_seq)

newdata_dt$predicted <- predict(WL2_bud_dt_fruit, newdata = newdata_dt, re.form = NA)

ggplot(WL2_dt_bud_fruit, aes(x = bud.date_dt, y = fruits, color = elev_m)) +
  geom_point(alpha = 0.8) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata_dt, aes(x = bud.date_dt, y = predicted),
...
ggplot(WL2_bud_dur_frt_prob, aes(x=bud.duration, y=WL2_Y2_Prob_Fruits, color=elev_m)) +
...
bud_seq <- seq(min(WL2_bud_dur_frt_prob$bud.duration, na.rm = TRUE),
               max(WL2_bud_dur_frt_prob$bud.duration, na.rm = TRUE))

newdata <- data.frame(bud.duration = bud_seq)

newdata$predicted <- predict(WL2_24_bud_dur_prob_fruit,
                             newdata = newdata,
                             type = "response",
                             re.form = NA)
...
ggplot(WL2_bud_dur_frt_prob, aes(x = bud.duration, y = WL2_Y2_Prob_Fruits, color = elev_m)) +
  ...
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata, aes(x = bud.duration, y = predicted),
            inherit.aes = FALSE,
            color = "blue", size = 1.2) +
  labs(x = "Bud Duration", y = "Survival to Fruiting")
...
ggplot(WL2_bud_dt_frt_prob, aes(x=bud.date_dt, y=WL2_Y2_Prob_Fruits, color=elev_m)) +
  geom_jitter(height=0.01) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0")
...
dt_bud_seq <- seq(min(WL2_bud_dt_frt_prob$bud.date_dt, na.rm = TRUE),
                  max(WL2_bud_dt_frt_prob$bud.date_dt, na.rm = TRUE))

newdata2 <- data.frame(bud.date_dt = dt_bud_seq)

newdata2$predicted <- predict(WL2_24_bud_dt_prob_fruit,
                              newdata = newdata2,
                              type = "response",
                              re.form = NA)
...
ggplot(WL2_bud_dt_frt_prob, aes(x = bud.date_dt, y = WL2_Y2_Prob_Fruits, color = elev_m)) +
  geom_jitter(height = 0.01, alpha = 0.8) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata2, aes(x = bud.date_dt, y = predicted),
            inherit.aes = FALSE,
            color = "blue", size = 1.2) +
  labs(x = "Days to Bud", y = "Survival to Fruiting")
```
...
all_WL2_fit_prob_bud_dur <- init.repro %>%
  filter(site == "WL2")  %>%
  filter(!(is.na(bud.duration))) %>%
  #left_join(WL2_prob_fit %>% select(bed, row, col, WL2_ProbFitness), by=c("bed", "row", "col")) %>%
  filter(!(pop %in% c("SQ1", "WR")))

ggplot(all_WL2_fit_prob_bud_dur, aes(x=bud.duration, y=WL2_ProbFitness, color=elev_m)) +
  geom_jitter(height=0.01) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0")
...
bud_dur_seq <- seq(min(all_WL2_fit_prob_bud_dur$bud.duration, na.rm = TRUE),
                   max(all_WL2_fit_prob_bud_dur$bud.duration, na.rm = TRUE))
newdata_pred <- data.frame(bud.duration = bud_dur_seq)
newdata_pred$predicted <- predict(prob_fit_bud_dur,
...
ggplot(all_WL2_fit_prob_bud_dur, aes(x = bud.duration, y = WL2_ProbFitness, color = elev_m)) +
  geom_jitter(height = 0.01, alpha = 0.8) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata_pred, aes(x = bud.duration, y = predicted),
            inherit.aes = FALSE,
            color = "blue", size = 1.2) +
  labs(x = "Bud Duration", y = "Probability of Making 1+ Fruits")
...
bud_dt_seq <- seq(min(all_WL2_fit_prob_bud_dur$bud.date_dt, na.rm = TRUE),
                   max(all_WL2_fit_prob_bud_dur$bud.date_dt, na.rm = TRUE))
newdata_pred2 <- data.frame(bud.date_dt = bud_dt_seq)
newdata_pred2$predicted <- predict(prob_fit_dt_bud,
                                   newdata = newdata_pred2,
                                   type = "response",
                                   re.form = NA)

ggplot(all_WL2_fit_prob_bud_dur, aes(x = bud.date_dt, y = WL2_ProbFitness, color = elev_m)) +
  geom_jitter(height = 0.01, alpha = 0.8) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata_pred2, aes(x = bud.date_dt, y = predicted),
            inherit.aes = FALSE,
            color = "blue", size = 1.2) +
  labs(x = "Days to bud", y = "Probability of Making 1+ Fruits")
```

#### WL2 total fitness - idk if we need this
...
WL2_total_fit_bud_dur <- init.repro %>%
  filter(site == "WL2") %>%
  #left_join(WL2_total_fit %>% select(bed, row, col, WL2_Total_Fitness, logTotalFitness), by=c("bed", "row", "col")) %>%
...
WL2_bud_dur_total_fit <- lmer(WL2_Total_Fitness ~ bud.duration + (1|pop),
                              data=WL2_total_fit_bud_dur)
summary(WL2_bud_dur_total_fit)
Anova(WL2_bud_dur_total_fit)

bud_dur_seq2 <- seq(min(WL2_total_fit_bud_dur$bud.duration, na.rm = TRUE),
                   max(WL2_total_fit_bud_dur$bud.duration, na.rm = TRUE))
newdata_fit2 <- data.frame(bud.duration = bud_dur_seq2)
newdata_fit2$predicted <- predict(WL2_bud_dur_total_fit,
                                 newdata = newdata_fit2,
                                 re.form = NA)

ggplot(WL2_total_fit_bud_dur, aes(x = bud.duration, y = WL2_Total_Fitness, color = elev_m)) +
  geom_point(alpha = 0.6) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_line(data = newdata_fit2, aes(x = bud.duration, y = predicted),
            inherit.aes = FALSE,
            color = "black", size = 1.2) +
  labs(x = "Bud Duration", y = "Total Fruit Produced") +
  theme_minimal()
